...
void main()
{
    ...
    // Durante o dia (lambert próximo 1) mostra só a textura diurna com luz direta
    // Durante a noite (lambert próximo 0) mostra as luzes da cidade
    // A transição é suave entre os dois

    vec3 colorDay = Kd0 * lambert;
    vec3 colorNight = cityLights * (1.0 - lambert);

    // Pequeno termo ambiente para o dia para não ficar totalmente preto
    float ambient = 0.01;

    // Cor final
    color.rgb = colorDay + colorNight + Kd0 * ambient;
